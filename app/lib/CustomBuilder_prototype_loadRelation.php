...
class CustomBuilder {
	...
	protected function loadRelation(array $models, $name, Closure $constraints)
	{
        ...
        /*****************************************************************************/
        /**
         * Получаем реальное имя отношения (relation) и его алиас
         */
        $alias = $name;
        if (mb_strpos($name, '|')) {
        ...
        /**
         * Вызываем одноименный метод из наследуемого класса
         */
        #$result = parent::loadRelation($models, $name, $constraints);
        /**
         * Обрабатываем полученную коллекцию - "переименовываем" ключ с полученным значением
         */
        /*
        if ($alias != $name) {
            #\Helper::d($alias);
            #\Helper::dd($result);
            #\Helper::dd($result[0]->relations);

            foreach ($result as $r => $res) {
                #\Helper::dd($res->getRelation($name));
                $res->$alias = $res->getRelation($name);
                #$res->loasRelation($alias) = $res->getRelation($name);
                unset($res->$name);
                $result[$r] = $res;
            }
        ...
        */
        ...
        $result = $relation->match($models, $results, $name);
        if ($alias == 'rrr') {
            #\Helper::d($alias);
            #\Helper::dd($result);
            #\Helper::dd($result[0]->relations);
            ...
            foreach ($result as $r => $res) {
                #\Helper::dd($res->getRelation($name));
                #$res->load((array)$res->getRelation($name));
                $res->$alias = $res->getRelation($name)->all();
                #unset($res->$name);
                $result[$r] = $res;
            }
